...
# Insert project root into sys.path so "package" is importable.
...
def test(n, k, m, dtype=np.double):
    ...
    # Row min/max
    check(A.pmin(axis=1), np.min(A_mat, axis=1, keepdims=True), "min(A, axis=1)")
    check(A.pmax(axis=1), np.max(A_mat, axis=1, keepdims=True), "max(A, axis=1)")


    # Global min/max (returns a scalar)
    check_scalar(A.pmin(), np.min(A_mat, keepdims=True), "min(A)")

    check_scalar(A.pmax(), np.max(A_mat, keepdims=True), "max(A)")
